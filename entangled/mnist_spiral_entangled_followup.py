"""
This is a version of the entangled method. We introduce an additional method of adding a sequence encoder and class-aware supervision so that hopefully the latent initialization and rollout would be better in preserving the digit identity across our frames.
Training structure and class organization was slightly slightly better.
"""
...
print("\n2. Training Improved Entangled Continuous-Time NODE...")
...
with torch.no_grad():
    ...
    for i in range(0, TRAIN_SEQS, MINI_BATCH):
        out = model(x_train_gt[i:i + MINI_BATCH], t_span)
        train_outputs.append(out["z0"])
    z0_train = torch.cat(train_outputs, dim=0)
    class_stats = {}
    for digit in DIGITS:
    ...
    sanity_out = model(x_train_gt[:2], t_span)
    sanity_roll = decode_to_image(sanity_out["rollout_logits"][:2, [0, T // 2, T - 1]])
...
plt.tight_layout()
fig_sanity.savefig("entangled_followup_sanity.png", dpi=150)
plt.close(fig_sanity)
print("   Saved 'entangled_followup_sanity.png'.")

print("\n3. Shared SPI Measurement Operator...")
A = torch.randint(0, 2, (T, M, H, W), device=device).float()
mask_norm = build_mask_normalizer(A)

print("\n4. Online Reconstruction with Improved Entangled NODE...")
results = {}
model.eval()
...
for digit in DIGITS:
    ...
    sched_z = optim.lr_scheduler.CosineAnnealingLR(opt_z, T_max=300, eta_min=1e-4)
    ...
    for step in range(300):
        ...
        if loss_total.item() < best_total:
            ...
            best_rollout = z_roll.detach().clone()
...
fig.suptitle("Improved Entangled Continuous-Time NODE Reconstruction", fontsize=14)
plt.tight_layout()
fig.savefig("reconstruction_entangled_followup_t31.png", dpi=150)
plt.close(fig)
print("   Saved 'reconstruction_entangled_followup_t31.png'.")

print("   Saving 'reconstruction_entangled_followup_t31.gif'...")
...
def update_followup(frame_idx: int):
    artists = []
    for row, digit in enumerate(DIGITS):
    ...
    fig_anim.suptitle(f"Entangled Follow-Up - Frame {frame_idx + 1}/{T}", fontsize=16)
    return artists


ani = animation.FuncAnimation(fig_anim, update_followup, frames=T, interval=250, blit=False)
ani.save("reconstruction_entangled_followup_t31.gif", writer="pillow")
plt.close(fig_anim)
print("   Saved 'reconstruction_entangled_followup_t31.gif'.")
